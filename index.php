...
<body>
	<div class="container mt-5 ">
		<center class="mb-5" ><h2>Data Guru</h2></center>
		<hr>
		<?php if (isset($_GET['pesan'])) { ?>
			<?php if ($_GET['pesan'] == "berhasil") { ?>
				<div class="alert alert-primary" role="alert">
					Berhasil Mengubah Data Guru
				</div>
		...
		<table class="table table-bordered mt-4" id="myTable">
			...
			<tbody>
				...
				$no = 1;
				$get_data = mysqli_query($koneksi,"SELECT * FROM guru");
				while ($data = mysqli_fetch_array($get_data)) {
					...
					<tr>
						...
						<td>
							...
							<?php }else{ ?>
								<img src="foto/<?php echo $data['foto_guru']; ?>" style="width:100px;height:100px;">
							<?php } ?>
						</td>
						<td>
							<a href="edit.php?id=<?php echo $data['id_guru'] ?>" class="btn btn-warning text-white">Edit</a>
							<a href="delete.php?id=<?php echo $data['id_guru'] ?>" class="btn btn-danger">Hapus</a>
						</td>
					</tr>
				<?php } ?>
			</tbody>
		</table>
		<a href="print.php?id=<?php echo $data['id_guru'] ?>" class="btn btn-success">print</a>
